...
const SaleToast: React.FC<{
  ...
  const timer = useCountdown(
    item.sale_ends_in ?? { hours: 20, minutes: 11, seconds: 15 },
  );
  return (
      ...
      <div className="flex gap-3 p-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[11px] text-gray-400 mt-1 text-center">
            Postage and tax will be calculated at checkout.
          </p>
        </div>
      </div>
    </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <Link
              ...
              className="text-[14px] text-gray-800 hover:underline underline-offset-2 leading-snug line-clamp-2"
            >
              {item.product_name}
            </Link>
            ...
            <div className="flex flex-wrap gap-1.5 mt-1.5">
              {item.variant_color && (
                <span className="text-[11px] text-gray-600 bg-gray-100 px-2.5 py-0.5 rounded-full border border-gray-200">
                  {item.variant_design
                    ? item.variant_design.toUpperCase()
                    : "COLOUR"}
                  : {item.variant_color}
                </span>
              )}
          ...
          {item.personalizations?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-1.5">
              {item.personalizations.map((p, i) => (
                <span
                  key={i}
                  className="text-[11px] text-gray-600 bg-gray-100 px-2.5 py-0.5 rounded-full border border-gray-200"
                >
          ...
          {timer &&
            item.sale_ends_in!.hours + item.sale_ends_in!.minutes > 0 && (
          ...
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-[16px] font-bold text-[#F1641E]">
              USD {item.price_per_unit.toFixed(2)}
            </span>
            {item.original_price &&
              item.original_price > item.price_per_unit && (
                <>
          ...
          <div className="flex items-center gap-3 mt-3 flex-wrap">
            <QuantitySelector
              value={item.quantity}
              onChange={(q) => onQuantityChange(item.client_id, q)}
            />
            <button className="text-[13px] text-gray-600 hover:text-gray-900 hover:underline underline-offset-2">
              Edit
            </button>
            <button
    ...
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-4 shadow-sm">
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
        <div className="flex items-center gap-2">
          ...
          <Link
            href={`/shop/${group.seller_name.toLowerCase().replace(/\s+/g, "-")}`}
              ...
              <span className="text-[12px] text-gray-600">
                ...
                {group.seller_review_count > 0 && (
                  <span className="text-gray-400 ml-0.5">
                    ({group.seller_review_count.toLocaleString()})
                  </span>
                )}
              </span>
            </div>
        ...
        <label className="flex items-center gap-2.5 cursor-pointer group">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[13px] text-gray-700">
              ...
              <button className="text-[#F1641E] hover:underline text-[12px]">
                Learn more
              </button>
            </span>
          </div>
...
const RelatedProductCard: React.FC<{
  ...
  <div className="group flex flex-col">
    <Link
      ...
      className="block relative aspect-square rounded-xl overflow-hidden bg-gray-100 mb-2"
    >
      {product.image ? (
        <Image
          src={product.image}
    ...
    <p className="text-[13px] text-gray-800 line-clamp-2 leading-snug mb-1">
      {product.title}
    </p>
    <div className="flex items-baseline gap-1.5 mb-1 flex-wrap">
      <span className="text-[14px] font-bold text-[#F1641E]">
    ...
    {product.has_free_delivery && (
      <span className="text-[11px] text-green-600 font-semibold mb-2">
        FREE delivery
      </span>
    )}
    <button
  ...
  <div className="sticky top-[88px]">
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-3 shadow-sm">
      ...
      <div className="relative flex items-center my-2">
        ...
        <span className="mx-3 text-[11px] text-gray-400 font-medium uppercase tracking-wide">
          or
        </span>
        <div className="flex-1 border-t border-gray-200" />
      </div>
    ...
    <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
      ...
      <div className="flex items-start gap-2 bg-gray-50 rounded-xl p-3 mb-4">
        ...
        <p className="text-[12px] text-gray-600 leading-relaxed">
          You&apos;re covered with{" "}
          <Link
            href="/etsy-purchase-protection"
            className="text-[#F1641E] hover:underline font-medium"
          >
      ...
      {cart.total_discount > 0 && (
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[14px] font-medium text-green-600">
            -USD {cart.total_discount.toFixed(2)}
          </span>
        </div>
      )}
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <span className="text-[14px] font-medium text-gray-900">
          {cart.total_shipping === 0
            ? "FREE"
            : `USD ${cart.total_shipping.toFixed(2)}`}
        </span>
      </div>
      ...
      <div className="border-t border-gray-200 pt-4 mb-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[18px] font-bold text-gray-900">
            USD {cart.grand_total.toFixed(2)}
          </span>
        </div>
        {cart.total_discount > 0 && (
          <p className="text-[12px] text-green-600 mt-1">
            You save USD {cart.total_discount.toFixed(2)}
          </p>
        )}
      </div>
      ...
      <div className="text-[11px] text-gray-500 space-y-1 border-t border-gray-100 pt-3">
        ...
        <p>
          * Learn more about additional taxes, duties, and fees that{" "}
          <button className="text-[#F1641E] hover:underline">may apply</button>
        </p>
      </div>
    </div>
  </div>
...
export default function BasketPage() {
  const { cart, removeItem, updateQuantity, addToCart, refreshCart } =
    useCart();
    ...
    (async () => {
      try {
        const res = await fetch(
          `${API}/products/?featured=true&in_stock=true&page_size=8`,
          { credentials: "include" },
        );
        ...
        const mock: RelatedProduct[] = [
          {
            id: 10,
            title: "Custom Embroidered P...",
            slug: "custom-p-1",
            price: 58.78,
            discount_price: 29.39,
            final_price: 29.39,
          ...
          },
          {
            ...
            discount_percentage: 0,
            image:
              "https://images.unsplash.com/photo-1543746986-2a585b4df0a6?w=400&h=400&fit=crop",
            rating: 4.7,
            review_count: 89,
            has_free_delivery: true,
          },
        ...
        setMoreRelated([
          {
            id: 20,
            title: "Custom Embroidered Pet Ph...",
            slug: "pet-ph-1",
            price: 19.99,
            discount_price: 7.0,
            final_price: 7.0,
          ...
          {
            id: 21,
            title: "Custom Embroidered Pet Ha...",
            slug: "pet-ha-1",
            price: 30.14,
            discount_price: 12.05,
            final_price: 12.05,
          ...
          {
            id: 22,
            title: "Custom Embroidered Pet Po...",
            slug: "pet-po-1",
            price: 14.52,
            discount_price: 5.81,
            final_price: 5.81,
            discount_percentage: 60,
            image:
              "https://images.unsplash.com/photo-1546961342-ea5f70a193fc?w=400&h=400&fit=crop",
            rating: 4.7,
            review_count: 445,
          },
          {
            id: 23,
            title: "Custom Dog Portrait Hat",
            slug: "dog-hat-1",
            price: 30.14,
            discount_price: 12.05,
            final_price: 12.05,
          ...
          {
            id: 24,
            title: "Custom EMBROIDERED Dog Cap",
            slug: "dog-cap-1",
            price: 21.5,
            discount_price: 9.03,
            final_price: 9.03,
  ...
  const handleAddRelated = async (slug: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const card = [...relatedProducts, ...moreRelated].find(
          (rp) => rp.slug === slug,
        );
        if (card) {
          await addToCart({
  ...
  // Get cart ID from cart context or localStorage
  const getCartId = () => {
    console.log("[BasketPage] Getting cart ID");
    // Try to get from cart context first
    // Try to get from cart context first
    if (cart.cart_id) {
      console.log("[BasketPage] Cart ID from context:", cart.cart_id);
      return cart.cart_id;
    }
    // Fall back to localStorage
    if (typeof window !== "undefined") {
      const storedCartId = localStorage.getItem("etsy_cart_id");
      if (storedCartId) {
        console.log("[BasketPage] Cart ID from localStorage:", storedCartId);
        return parseInt(storedCartId, 10);
      }
    }
    ...
    <>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-8 pb-20">
        ...
        {isEmpty ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 mb-6">
              Start exploring to find unique items you&apos;ll love.
            </p>
            <Link
              href="/"
              className="inline-block px-8 py-3 bg-[#F1641E] text-white rounded-full font-semibold hover:bg-[#d95518] transition-colors"
            >
              Continue shopping
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 xl:gap-8">
            ...
            <div>
                ...
                <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-4 shadow-sm">
                  <h3 className="text-[15px] font-semibold text-gray-900 mb-1">
                    ...
                    <span className="text-gray-400 font-normal text-[13px]">
                      Including ads{" "}
                      <button aria-label="About ads">
                        <Info size={13} className="inline text-gray-400" />
                      </button>
                    </span>
                  </h3>
              ...
              <div className="flex items-center gap-2 py-4 px-1">
                ...
                <p className="text-[13px] text-gray-600">
                  Etsy invests in climate solutions like electric trucks and
                  carbon offsets for every delivery.{" "}
                  <button className="text-[#F1641E] hover:underline">
                    See how
                  </button>
                </p>
              </div>
            ...
            <OrderSummaryPanel cart={cart} onCheckout={openCheckoutModal} />
          </div>
        )}
        ...
        <div className="mt-12 pt-6 border-t border-gray-100 text-center text-[11px] text-gray-400 space-y-2">
          ...
          <p>
            Etsy, Inc., USA 117 Adams Street Brooklyn, NY 11201&nbsp;&nbsp;Etsy
            Ireland UC One Le Pole Square Ship Street Great Dublin 8
          </p>
        </div>
      </div>
    </>
