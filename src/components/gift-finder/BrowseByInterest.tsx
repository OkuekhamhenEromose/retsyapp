...
const BrowseByInterest = ({ interests, initialCollections }: BrowseByInterestProps) => {
  const [selectedInterest, setSelectedInterest] = useState("Jewellery");
  const [collections, setCollections] = useState<GiftCollection[]>(initialCollections);
  ...
  useEffect(() => {
    const fetchCollections = async () => {
      if (selectedInterest) {
        setLoading(true);
        try {
          const data = await apiService.getCollectionsByInterest(selectedInterest);
          setCollections(data);
        } catch (error) {
            ...
            <div key={collection.id} className="mb-12">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-full ${collection.persona.bg_color || 'bg-gray-200'} flex items-center justify-center`}>
                    <span className="text-xl">{collection.persona.icon || '🎁'}</span>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">{collection.persona.name}</p>
                    <h3 className="font-bold text-gray-900">{collection.title}</h3>
                  </div>
                </div>
                <Link 
                  href={`/gift-collections/${collection.slug}`} 
              ...
              <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
                {collection.products.map((product: GiftProduct) => (
                  ...
                  >
                    ...
                    <p className="text-xs font-medium truncate text-gray-900">{product.title}</p>
                    <div className="flex items-center gap-1 text-xs">
                      <span className="text-yellow-500">{"★".repeat(Math.floor(product.rating))}</span>
                      <span className="text-gray-600">({formatReviewCount(product.review_count)})</span>
                    </div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className={`text-sm font-medium ${product.discount_price ? 'text-green-700' : 'text-gray-900'}`}>
