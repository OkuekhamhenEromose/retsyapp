...
const MOCK_PURCHASES = [
  { id: 1, shopName: 'VintageFindsShop', orderId: '2847391023', date: 'Feb 15, 2026', status: 'Delivered' as const, total: 34.99, items: [{ name: 'Handmade Silver Bracelet',  price: 34.99, image: IMGS.jewellery,  reviewed: false, rating: 0 }] },
  { id: 2, shopName: 'ModernHomeDecor',  orderId: '2847391024', date: 'Feb 10, 2026', status: 'Delivered' as const, total: 89.00, items: [{ name: 'Minimalist Table Lamp',      price: 89.00, image: IMGS.homeLiving, reviewed: true,  rating: 5 }] },
  { id: 3, shopName: 'KidsPlayWorld',    orderId: '2847391025', date: 'Jan 28, 2026', status: 'Delivered' as const, total: 24.50, items: [{ name: 'Animal Costume Mask Set',    price: 24.50, image: IMGS.kids,       reviewed: false, rating: 0 }] },
];
...
const ReviewModal: React.FC<ReviewModalProps> = ({ itemName, itemImage, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 z-[500] flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-[460px] bg-white rounded-2xl shadow-2xl p-7 z-10" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-gray-100 hover:text-gray-700 transition-colors">
          <X size={16} />
        </button>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-3 py-3 mb-5">
              <img src={itemImage} alt={itemName} className="w-12 h-12 rounded-lg object-cover shrink-0 border border-gray-200" />
              <p className="text-[13px] font-medium text-gray-800 leading-snug line-clamp-2">{itemName}</p>
            </div>
              ...
              <div className="flex gap-1">
                {[1,2,3,4,5].map((s) => (
                  <button key={s} type="button"
                    ...
                    <Star size={28} fill={(hovered || rating) >= s ? '#F1641E' : 'none'} stroke={(hovered || rating) >= s ? '#F1641E' : '#d1d5db'} strokeWidth={1.5} />
                  </button>
                ))}
              </div>
              {(hovered || rating) > 0 && <p className="text-[12px] text-gray-400 mt-1">{labels[hovered || rating]}</p>}
            </div>
            ...
            <div className="mb-5">
              ...
              <textarea
                value={text} onChange={(e) => setText(e.target.value)} rows={4} maxLength={3000}
            ...
            <div className="flex gap-2.5">
              <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-full border border-gray-300 text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors">Cancel</button>
              <button type="submit" disabled={!rating} className="flex-1 py-2.5 rounded-full bg-[#F1641E] hover:bg-[#d4571a] text-white text-[13px] font-semibold transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Submit review</button>
            </div>
          </form>
        )}
      </div>
...
const EmptyState: React.FC = () => (
  <div className="border border-gray-200 rounded-2xl bg-white px-8 py-10 mx-auto max-w-[920px]">
    ...
    <div className="grid grid-cols-4 gap-4 mb-0">
      {BROWSE_CATEGORIES.slice(0, 4).map((cat) => (
        <button key={cat.name} className="group flex flex-col items-center gap-2 text-left">
          <div className="w-full aspect-square overflow-hidden">
            <img src={cat.image} alt={cat.name} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-300" />
          </div>
          <span className="text-[14px] text-gray-900 font-normal self-start">{cat.name}</span>
        </button>
      ))}
    </div>
    ...
    <div className="grid grid-cols-4 gap-4 mt-4">
      <button className="group flex flex-col items-center gap-2 text-left">
        <div className="w-full aspect-square overflow-hidden">
          <img src={BROWSE_CATEGORIES[4].image} alt={BROWSE_CATEGORIES[4].name} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-300" />
        </div>
        <span className="text-[14px] text-gray-900 font-normal self-start">{BROWSE_CATEGORIES[4].name}</span>
      </button>
      <div /><div /><div />
    </div>
  </div>
...
interface PurchaseItem { name: string; price: number; image: string; reviewed: boolean; rating: number }
interface Purchase { id: number; shopName: string; orderId: string; date: string; status: PurchaseStatus; total: number; items: PurchaseItem[] }
...
const PurchaseCard: React.FC<{ purchase: Purchase; submittedReviews: Record<string, boolean>; onReview: (name: string, image: string) => void }> = ({ purchase, submittedReviews, onReview }) => (
  <div className="border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
    <div className="flex items-center justify-between bg-gray-50 px-5 py-3 border-b border-gray-200">
      <div className="flex items-center gap-3">
        ...
        <span className="hidden sm:inline text-[12px] text-gray-400">Order #{purchase.orderId} · {purchase.date}</span>
      </div>
      <div className="flex items-center gap-3">
        <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${STATUS_STYLES[purchase.status]}`}>{purchase.status}</span>
        <span className="text-[13px] font-semibold text-gray-800">USD {purchase.total.toFixed(2)}</span>
      </div>
    </div>
    ...
    {purchase.items.map((item, idx) => {
      ...
      return (
        <div key={idx} className="flex items-center gap-5 px-5 py-4 border-b border-gray-100 last:border-b-0">
          <img src={item.image} alt={item.name} className="w-[72px] h-[72px] object-cover rounded-lg border border-gray-200 shrink-0" />
          ...
          <div className="flex items-center gap-2 shrink-0">
            {isReviewed ? (
              <div className="flex items-center gap-1 text-[12px] text-gray-500">
                <div className="flex">
                  {[1,2,3,4,5].map((s) => (
                    <Star key={s} size={13} fill={s <= (item.rating || 5) ? '#F1641E' : 'none'} stroke={s <= (item.rating || 5) ? '#F1641E' : '#d1d5db'} strokeWidth={1.5} />
                  ))}
                </div>
                <span className="ml-0.5">Reviewed</span>
              </div>
            ) : (
              <button onClick={() => onReview(item.name, item.image)} className="px-4 py-2 text-[12px] font-medium text-gray-900 border border-gray-900 rounded-full hover:bg-gray-900 hover:text-white transition-colors whitespace-nowrap">
                Write a review
              </button>
            )}
    ...
    <div className="flex items-center gap-4 px-5 py-2.5 bg-gray-50/60 border-t border-gray-100">
      {[
        ...
        { icon: MessageSquare, label: 'Contact shop'  },
...
// ── PurchasesSection ──────────────────────────────────────────────────────────
interface PurchasesSectionProps { hasPurchases?: boolean }

const PurchasesSection: React.FC<PurchasesSectionProps> = ({ hasPurchases = false }) => {
    ...
    <>
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 mt-10 mb-20">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-3">
          <h1 className="text-[30px] sm:text-[34px] font-light text-gray-900 leading-none font-serif">
            Purchases
          </h1>
          <div className="flex items-center gap-6 sm:gap-8">
            ...
            <div className="flex items-center gap-5">
              {(['purchases', 'cases'] as const).map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`text-[14px] pb-0.5 transition-colors capitalize ${activeTab === tab ? 'text-gray-900 font-semibold border-b-2 border-gray-900' : 'text-gray-500 font-medium hover:text-gray-800'}`}>
            ...
            <div className="relative">
              <input
                ...
                className="h-[42px] w-52 sm:w-[260px] pl-4 pr-11 border border-gray-300 text-[13px] text-gray-800 placeholder-gray-400 bg-white focus:outline-none focus:border-gray-700 transition-colors rounded-sm"
              />
              <button aria-label="Search" className="absolute right-0 top-0 h-[42px] w-[42px] flex items-center justify-center text-gray-600 hover:text-gray-900 transition-colors">
                <Search size={17} />
              </button>
            </div>
          </div>
          ...
          <>
            {!hasPurchases ? (
              ...
              <div className="bg-white border border-gray-200 rounded-xl px-8 py-14 text-center">
                ...
                <p className="text-gray-600 font-medium">No results for &ldquo;{searchQuery}&rdquo;</p>
                <p className="text-[13px] text-gray-400 mt-1">Try a different search term.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-5">
                {filteredPurchases.map((p) => (
                  <PurchaseCard key={p.id} purchase={p} submittedReviews={submittedReviews} onReview={(name, image) => setReviewTarget({ name, image })} />
                ))}
              </div>
            )}
          </>
      ...
      {reviewTarget && (
        <ReviewModal
          itemName={reviewTarget.name} itemImage={reviewTarget.image}
          onClose={() => setReviewTarget(null)}
          onSubmit={(name) => { setSubmittedReviews((prev) => ({ ...prev, [name]: true })); setReviewTarget(null); }}
        />
      )}
    </>
  );
